...
from models.pointnet2_cls_ssg import get_loss
...
class PointNetTrainer(Trainer):
    ...
    def train_on_episode(self, model, optim, ds_train, ds_test=None):
        ...
        for it in range(self.config['training']['num_train_steps_per_episode']):
            ...
            pred, trans_feat = model(x)  # [batch_size, num_classes_per_task]
            # predd, trans_featt = model(xx)

            loss = F.nll_loss(pred, y)
            # loss = F.nll_loss(predd, yy)  # get_loss(pred, y, trans_feat)
            ...
            # print(correct)
            accs.append(correct.item() / float(len(x)))
    ...
    def compute_scores(self, model, dataset) -> Tuple[np.float, np.float]:
        ...
        pred, trans_feat = model(x)
        loss = F.nll_loss(pred, y).item()
        pred_choice = pred.data.max(1)[1]

        correct = pred_choice.eq(y.long().data).cpu().sum()
        # print(correct)

        acc = correct.item() / float(len(x))
